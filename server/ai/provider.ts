...
function aiProductSearchNormalizationInstructions() {
  return [
    ...
    "Candidate count rule: if one interpretation is clearly dominant, return exactly one primary HS4/HS6/HS10 direction. Do not add alternatives merely to fill a list. If the input is ambiguous, return only the materially different interpretations needed to ask the next question.",
    ...
    "For needs_clarification, still include one primaryCandidate and one candidateHsCodes item when a broad HS4/HS6 boundary is reasonably inferable. Leave candidateHsCodes empty only when the missing facts can change the chapter/heading so much that even a broad boundary would mislead the user.",
    ...
    "When certainty is medium or low because of multiple meanings, provide candidateHsCodes as 2 to 6 plausible HS heading/subheading/code prefixes. When certainty is medium or low because branch facts are missing for one product family, provide only the best broad boundary and missingQuestions. Order alternatives by product-context likelihood. Prefer HS6 prefixes; use HS4 when only the heading is reasonably inferable. Use HS10 only when the input clearly names a narrow commodity.",
