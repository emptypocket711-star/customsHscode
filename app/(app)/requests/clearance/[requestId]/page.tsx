...
export default async function ClearanceRequestDetailPage({
  params
}: {
  ...
  const request = await getOwnClearanceRequest(supabase, requestId);
  ...
  if (!request.schemaReady) {
    return (
      <div className="grid gap-5">
        <PageHeading
          title="통관 의뢰 요청 상세"
          description="로그인은 정상입니다. 현재 이 환경에서는 플랫폼 요청 상세 데이터가 준비되지 않아 통관 의뢰 상세 작업을 불러올 수 없습니다."
        />
      </div>
    );
  }
  ...
  const [receivedBids, requestDocuments, requestQuestions, feedbackByRequestId, completionReports] = await Promise.all([
    listReceivedClearanceBids(supabase, [request.item.id]),
    listClearanceRequestDocuments(supabase, [request.item.id]),
    listClearanceRequestQuestions(supabase, [request.item.id]),
  ...
  return (
    <div className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <PageHeading
          title="통관 의뢰 요청 상세"
          description="이 요청의 서류, 질문, 통관 견적 비교, 관세사무소 선정 작업을 한 화면에서 처리합니다."
        />
        <Link
      ...
      <div className="grid gap-3 rounded-md border border-slate-200 bg-white p-3 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-slate-600">{nextFocus.detail}</p>
        </div>
