...
export default async function FreightOpportunityDetailPage({
  ...
  const opportunity = await getMatchedFreightOpportunity(supabase, requestId);
  ...
  if (!opportunity.schemaReady) {
    return (
      <div className="grid gap-5">
        <PageHeading
          title="운송 입찰 작업"
          description="로그인은 정상입니다. 현재 이 환경에서는 포워더 입찰 데이터가 준비되지 않아 입찰 작업을 불러올 수 없습니다."
        />
      </div>
    );
  }
  ...
  const [requestDocuments, requestQuestions, ownBids, feedbackByRequestId, completionReports] = await Promise.all([
    ...
    listReceivedFreightBids(supabase, [opportunityItem.id]),
  ...
  return (
    <div className="grid gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <PageHeading
          title="운송 입찰 작업"
          description="공개된 운송 조건, 서류, 질문 답변을 확인하고 포워더 견적을 제출합니다."
        />
        <Link
