...
- 실제 외부 이메일, 문자, 푸시 provider는 아직 연결하지 않았다.
...
그 외 provider 값은 지원하지 않는다. 실제 이메일, 문자, 푸시 provider를 추가하기 전까지는 `MARKETPLACE_NOTIFICATIONS_PROVIDER`에 다른 값을 넣어도 발송 준비 완료로 보지 않는다.
...
- 실제 이메일/문자/푸시 provider를 연결하기 전에는 운영 사용자에게 발송된 것으로 안내하면 안 된다.
