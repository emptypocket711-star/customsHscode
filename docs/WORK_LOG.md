...
- 기본 4개 카드의 라벨을 `요청 규모 확인`, `견적 선택 대기`, `완료 전환 대기`, `후기 품질 확인`으로 변경했다.
- 각 카드 설명에 `선택 전환`, `완료 전환`, `파트너 비교 기준`처럼 운영자가 바로 요청할 수 있는 행동 단어를 넣었다.
- 다음 작업은 P220 operations request detail priority view다. 이번 P219가 통계 카드 라벨을 행동 중심으로 바꾼 작업이라면, P220은 운영 요청 상세에서 완료 리포트, 피드백, 분쟁 위험, 다음 조치를 먼저 보이게 하는 작업이다.
...
- `buildCompletionReportWorkflow`에 `requesterAcknowledged`, `partnerAcknowledged` 입력을 추가했다.
- 완료 리포트 패널은 `sourceSnapshot.confirmations.requester/partner`를 읽어 workflow에 전달한다.
...
- `npx vitest run features/service-requests/service-request-completion-report-workflow.test.ts server/repositories/platform-marketplace-governance.test.ts`: 35개 통과
- `npm run typecheck`
- `npm run lint`
- `npm run build`
- `supabase migration lint`: 현재 CLI 미지원으로 help만 출력됨
- `VERCEL_AUTOMATION_BYPASS_SECRET=... SMOKE_REQUIRE_AUTHENTICATED=true npm run smoke:production -- https://customs-hscode-ivdkom0x4-koo-apps.vercel.app`: 9/9 통과
- `VERCEL_AUTOMATION_BYPASS_SECRET=... npm run smoke:operations:guard -- https://customs-hscode-ivdkom0x4-koo-apps.vercel.app`: 9/9 통과
