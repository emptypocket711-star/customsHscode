...
1. route별 실패율과 평균 응답시간을 운영 화면에 표시
2. Vercel/cron/job 실행 실패를 개발자 공지 또는 운영 알림으로 노출
3. 반입계 출력 Playwright 실패 원인별 통계화
...
1. KOTRA 뉴스 국가 필터와 요약 카드 품질 개선
2. API key와 endpoint 설정 상태를 운영 화면에서 더 명확히 구분
3. 터미널 반입계 출력 실패 원인별 통계와 재시도 안내 개선
4. API001/API012 정기 job 실패 이력을 운영 화면에 노출
