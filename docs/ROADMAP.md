...
| P193.1 company settings role guidance | 완료 | 저장 실패 메시지가 아니라 회사 설정 화면에서 화주 역할과 회사 정보 확인 위치를 바로 이해할 수 있게 상단 설명과 다음 작업 문구를 보강한다 | company settings role guidance | browser, typecheck, lint, build, staging smoke |
...
| P204.1 operations smoke content markers | 완료 | 운영자 계정 준비가 아니라 운영자 smoke가 켜졌을 때 각 운영 화면의 고유 본문 문구를 확인하도록 marker를 강화한다 | operations smoke content markers | node check, typecheck, lint, build, staging smoke |
| P205.1 operations smoke diagnostics | 완료 | 운영자 본문 marker가 아니라 운영자 smoke가 비활성일 때 어떤 env가 빠졌는지 결과 로그로 확인하게 한다 | operations smoke diagnostics | node check, typecheck, lint, build, staging smoke |
...
| P218.1 operations home action rail | 완료 | 운영 패널 내부 요약이 아니라 운영 관리 홈 첫 화면에서 역할 신청, 회사 검증, 플랫폼 병목 중 오늘 먼저 볼 레일을 더 선명하게 만든다 | operations home simplification | platform bottleneck rail, unit test |
...
| P255.1 publish RPC nullable company guard | 완료 | 테스트 하네스가 아니라 회사 없는 인증 사용자가 운송 요청 공개 RPC를 직접 호출해 권한 검사를 우회하지 못하게 한다 | publish auth guard | migration governance, RLS negative test, typecheck, lint, build |
...
| P275.1 partner preference anchor smoke | 완료 | zero-match smoke가 아니라 파트너 빈 상태의 관심 조건 링크와 settings anchor를 브라우저 스모크에 고정한다 | preference anchor smoke | route/browser smoke |
