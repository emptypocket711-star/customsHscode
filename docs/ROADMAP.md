...
1. 실제 검색 실패 케이스를 telemetry에서 유형별로 묶기
2. “제품명/브랜드명/모델명만 입력” 시 웹 근거 또는 보완질문으로 분기
3. GPT 응답이 HS6만 줄 때 하위 HSK 선택 흐름 개선
...
1. 외부 연동별 timeout, 재시도, 사용자 오류 문구 표준화
2. 터미널 반입계 출력 실패 원인별 안내 개선
3. KOTRA 뉴스 국가 필터와 요약 카드 품질 개선
4. API key와 endpoint 설정 상태를 운영 화면에서 더 명확히 구분
