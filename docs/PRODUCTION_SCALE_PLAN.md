...
- Product-name GPT normalization waits for the primary GPT response within the server execution budget (`OPENAI_PRODUCT_SEARCH_TIMEOUT_MS`, recommended 30 seconds) and does not attach live web-search tools. The prompt asks for compact JSON only, so user wait time is driven by the GPT completion rather than a long explanation. A no-candidate GPT retry uses `OPENAI_PRODUCT_SEARCH_RETRY_TIMEOUT_MS` (recommended 8 seconds), and no-candidate/no-primary normalization results are not cached. Model numbers, SKUs, trade names, and short Korean product names are interpreted from visible input only; if the product cannot be identified, the app asks for category, use, material, catalog, photo, or specification details instead of performing web lookup.
