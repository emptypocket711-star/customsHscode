    ...
    return (
      <div className="grid scroll-mt-6 gap-3" id="request-completion">
        <div className="grid gap-3 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
          <p className="font-semibold text-emerald-950">통관 의뢰 완료 후 확인 순서</p>
          <div className="grid gap-2 text-xs leading-5 md:grid-cols-3">
            <p className="rounded-md bg-white p-2">1. 완료 리포트의 신고 결과와 예비 조회 출처 구분을 확인합니다.</p>
...
export function ClearanceRequestRow({
  ...
  return (
    <div className="grid gap-3 rounded-md border border-slate-200 bg-white p-4">
      ...
      <ClearanceLifecycleControls compact={compact} completionReport={completionReport} completionReportDocuments={completionReportDocuments} documents={documents} existingFeedback={feedbackByRequestId[request.id]} requestId={request.id} status={request.status} viewerRole="requester" />
...
export function ClearanceOpportunityRow({
  ...
  return (
    <div className="grid gap-4 rounded-md border border-slate-200 bg-white p-4">
      ...
      {compact ? (
        <p className="rounded-md border border-slate-200 bg-slate-50 p-3 text-xs leading-5 text-slate-600">
          공개 서류 {documents.length}건 / 질문 {questions.length}건입니다. {opportunity.interestStatus === "declined" ? "참여 보류로 저장된 요청입니다. 다시 참여하려면 입찰 작업에서 상태를 확인합니다." : opportunity.status === "partner_selected" || opportunity.status === "in_progress" || opportunity.status === "completed" ? "선정 후 업무는 입찰 작업에서 확인합니다." : "조건 확인, 질문 등록, 통관 견적 제출은 입찰 작업에서 처리합니다."}
        </p>
      ) : null}
      {compact ? null : (
        <>
        ...
        <CardBody>
          ...
          {!schemaReady ? (
            <div className="mb-4 grid gap-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-900">
              <p>로그인은 정상입니다. 현재 이 환경에서는 플랫폼 요청 기능이 준비 중이라 통관 의뢰 초안 저장과 관세사무소 공개 모집이 제한됩니다.</p>
          ...
          >
            ...
            {state.message ? (
              <div className={state.status === "success" ? "rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800" : "grid gap-3 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-800"}>
                <p>{state.message}</p>
                {state.status === "error" ? (
                  <Link className="focus-ring inline-flex h-9 w-fit items-center justify-center rounded-md bg-white px-3 text-xs font-semibold text-red-800 ring-1 ring-red-200 hover:bg-red-100" data-navigation-progress="회사 설정" href="/settings/members#platform-role-request">
                    회사 설정 확인
                  </Link>
                ) : null}
              </div>
        ...
        <CardBody className="grid gap-3">
          ...
          {!schemaReady ? (
            <div className="grid gap-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-900">
              <p>현재 이 환경에서는 관세사무소 입찰 데이터가 준비되지 않아 입찰 가능 요청을 불러올 수 없습니다. 역할 권한 확인은 플랫폼 데이터 준비 후 진행합니다.</p>
              <RoleRequestSettingsLink />
            </div>
          ) : null}
