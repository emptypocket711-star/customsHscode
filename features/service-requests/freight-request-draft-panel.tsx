...
function FreightOwnSubmittedBidSummary({ bid }: { bid: ReceivedFreightBidItem }) {
  return (
    <div id="opportunity-bid" className="scroll-mt-6 grid gap-2 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-xs leading-5 text-emerald-950">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm font-semibold">제출한 운송 견적</p>
    ...
    return (
      <form action={startAction} className="grid scroll-mt-6 gap-3 rounded-md border border-emerald-200 bg-emerald-50 p-3 md:grid-cols-[1fr_auto] md:items-center" id="request-lifecycle">
        ...
        <div className="grid gap-1 text-sm leading-6 text-emerald-950">
          ...
          <p>
            {viewerRole === "partner"
              ? "화주와 선적 가능 일정, 비용 포함 범위, 필요 서류 전달 방식을 확인한 뒤 운송 진행을 시작합니다."
              : "선정 포워더와 선적 일정, 비용 포함 범위, 선정 파트너 전용 서류 공개 범위를 확인한 뒤 운송 진행을 시작합니다."}
          </p>
        </div>
    ...
    return (
      <form action={completeAction} className="grid scroll-mt-6 gap-3 rounded-md border border-slate-200 bg-slate-50 p-3" id="request-lifecycle">
        ...
        <div className="grid gap-1 text-sm leading-6 text-slate-700">
          ...
          <p>운송 완료 여부, 최종 보관 서류, 정산 요약에 남길 민감정보 제외 메모를 확인한 뒤 완료 처리합니다.</p>
        </div>
...
export function FreightOpportunityRow({
  ...
  ownSubmittedBid?: ReceivedFreightBidItem;
  questions?: FreightRequestQuestionItem[];
}) {
  const router = useRouter();
  ...
  return (
    <div className="grid gap-3 rounded-md border border-slate-200 bg-white p-4">
      ...
      {ownSubmittedBid ? (
        <FreightOwnSubmittedBidSummary bid={ownSubmittedBid} />
...
export function FreightRequestDraftPanel({
  ...
  const statusFilterOptions = serviceRequestStatusFilterOptions(counts, activeWorkspace === "forwarder" ? "partner" : "requester");
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="flex flex-wrap gap-2 rounded-md border border-slate-200 bg-white p-3" aria-label="운송 요청 상태 필터">
          ...
          <Card id="my-service-requests">
            ...
            <CardBody className="grid gap-3">
              ...
              {schemaReady && requests.length > 0 && filteredRequests.length === 0 ? (
                <p className="rounded-md border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                  선택한 상태({serviceRequestStatusFilterLabel(statusFilter)})에 해당하는 운송 견적 요청이 없습니다.
                </p>
              ) : null}
          ...
          <CardBody className="grid gap-3">
            ...
            {schemaReady && opportunities.length === 0 ? (
              <div className="grid gap-3 rounded-md border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                <p>{partnerOpportunityEmptyCopy.freight}</p>
                <Link
            ...
            {schemaReady && opportunities.length > 0 && filteredOpportunities.length === 0 ? (
              <p className="rounded-md border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                선택한 상태({serviceRequestStatusFilterLabel(statusFilter)})에 해당하는 입찰 가능 운송 요청이 없습니다.
              </p>
            ) : null}
