...
function buildCopyReadyOperationsRequest(
  ...
  const lines = [
    ...
    `- 피드백 수: ${summary.feedbackCount}건`,
    `- 평균 후기: ${summary.averageFeedbackRating ?? "-"}점`,
    `- 낮은 후기: ${summary.lowFeedbacks}건`,
    `- 완료 리포트 없음: ${summary.completedWithoutReport}건`,
    `- 완료 리포트 확인 대기: ${summary.completionReportsSubmitted}건`,
    `- 완료 리포트 운영 검토 필요: ${summary.completionReportsAcknowledged}건`,
    `- 완료 리포트 잠금 대기: ${summary.completionReportsReadyToLock}건`,
    `- 완료 리포트 잠금 완료: ${summary.completionReportsLocked}건`,
    `- 완료 후 피드백 없음: ${summary.completedWithoutFeedback}건`,
    `- 미답변 질문: ${summary.unansweredQuestions}건`,
    `- 견적 없는 공개: ${summary.openWithoutBids}건`,
    `- 오래된 초안: ${summary.staleDrafts}건`,
    `- 오래 진행중: ${summary.staleInProgress}건`,
    `- 마감 지난 공개: ${summary.staleOpen}건`,
    "",
    "확인 샘플:",
