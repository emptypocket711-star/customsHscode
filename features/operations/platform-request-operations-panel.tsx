...
function buildOwnerActionQueue(summary: PlatformRequestOperationsSummary) {
  ...
  const queue = [
    ...
    },
    {
      ...
      detail: `알림이 전달됐는데도 견적이 없으면 원인 단서부터 봅니다. 질문 ${summary.notifiedWithoutBidsWithUnansweredQuestions}건 / 서류 없음 ${summary.notifiedWithoutBidsWithoutDocuments}건 / 파트너 활동 ${summary.notifiedWithoutBidsPartnerActivity}건 / 미열람 추정 ${summary.notifiedWithoutBidsPartnerUnseen}건입니다.`,
      label: "알림 후 무응답",
      owner: "운영자",
      reason: "파트너 응답 지연",
      tone: "warning" as const
...
function buildCopyReadyOperationsRequest(
  ...
  const lines = [
    ...
    `- 무응답 원인 단서: 미답변 질문 ${summary.notifiedWithoutBidsWithUnansweredQuestions}건 / 공개 서류 없음 ${summary.notifiedWithoutBidsWithoutDocuments}건 / 파트너 활동 ${summary.notifiedWithoutBidsPartnerActivity}건 / 미열람 추정 ${summary.notifiedWithoutBidsPartnerUnseen}건`,
...
export function buildPlatformRequestOperationsMetricGroups(summary: PlatformRequestOperationsSummary): {
  ...
  return {
    diagnosticMetrics: [
      ...
      { detail: "알림 후 무응답 중 화주 답변이 필요한 질문이 남은 요청", label: "무응답·질문", tone: tone(summary.notifiedWithoutBidsWithUnansweredQuestions), value: `${summary.notifiedWithoutBidsWithUnansweredQuestions}건` },
      { detail: "알림 후 무응답 중 공개 서류가 없는 요청", label: "무응답·서류 없음", tone: tone(summary.notifiedWithoutBidsWithoutDocuments), value: `${summary.notifiedWithoutBidsWithoutDocuments}건` },
      { detail: "알림 후 무응답 중 파트너가 열람·관심·보류 상태를 남긴 요청", label: "무응답·파트너 활동", tone: tone(summary.notifiedWithoutBidsPartnerActivity), value: `${summary.notifiedWithoutBidsPartnerActivity}건` },
      { detail: "알림은 발송됐지만 파트너 활동 상태가 없는 요청", label: "무응답·미열람 추정", tone: tone(summary.notifiedWithoutBidsPartnerUnseen), value: `${summary.notifiedWithoutBidsPartnerUnseen}건` },
