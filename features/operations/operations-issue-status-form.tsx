...
function statusButtonClassName(status: OperationsIssueStatus) {
  if (status === "resolved") {
    return "rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-800 transition hover:bg-emerald-100 disabled:cursor-not-allowed disabled:opacity-60";
  }

  if (status === "open") {
    return "rounded-md border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60";
  }
      ...
      <div className="flex flex-wrap gap-2">
        {nextStatuses.map((status) => (
          <button
            ...
            value={status}
          >
            {pending ? "저장 중" : statusButtonLabel(status)}
          </button>
        ))}
      </div>
      {state.message ? (
