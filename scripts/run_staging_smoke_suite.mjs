...
export function createStagingSmokeSteps(targetBaseUrl = baseUrl) {
  return [
    {
      label: "health-db",
      command: "npm",
    ...
    },
    {
      ...
      command: "npm",
      args: ["run", "smoke:operations:guard", "--", targetBaseUrl],
    ...
    },
    {
      ...
      command: "npm",
      args: ["run", "e2e:marketplace-transaction:staging", "--", targetBaseUrl],
    ...
    },
    {
      ...
      command: "npm",
      args: ["run", "e2e:completion-preview:staging", "--", targetBaseUrl],
    ...
    },
    {
      ...
      command: "npm",
      args: ["run", "e2e:marketplace-notification:staging", "--", targetBaseUrl],
    ...
    },
    {
      ...
      command: "npm",
      args: ["run", "ops:marketplace-notifications:rehearse-staging", "--", targetBaseUrl],
...
export function loadShipperAccountDefaults(env = process.env) {
  const fixturePassword = env.E2E_TEST_PASSWORD || "";
  if (fixturePassword) {
    return {
      E2E_TEST_PASSWORD: fixturePassword,
      SMOKE_LOGIN_EMAIL: env.SMOKE_LOGIN_EMAIL || marketplaceTransactionFixture.users.requester.email,
      SMOKE_LOGIN_PASSWORD: env.SMOKE_LOGIN_PASSWORD || fixturePassword
    };
  }
  ...
  try {
    ...
    return {
      E2E_TEST_PASSWORD: env.E2E_TEST_PASSWORD || shipper.password,
      SMOKE_LOGIN_EMAIL: env.SMOKE_LOGIN_EMAIL || shipper.email,
      SMOKE_LOGIN_PASSWORD: env.SMOKE_LOGIN_PASSWORD || shipper.password
    };
  } catch {
    return {};
  }
...
function main() {
  ...
  const results = [];
  const steps = createStagingSmokeSteps(baseUrl);
