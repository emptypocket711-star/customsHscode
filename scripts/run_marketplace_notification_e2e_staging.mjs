...
const storageStateDir = process.env.E2E_STORAGE_STATE_DIR || "tmp/e2e-auth-notification-staging";

process.env.E2E_ALLOW_REMOTE_MARKETPLACE_TRANSACTION = "true";
process.env.E2E_ALLOW_REMOTE_MARKETPLACE_NOTIFICATION = "true";
...
function fixtureEnv() {
  return {
    ...
    [fixture.notification.envKeys.deliveryId]: fixture.notification.delivery.id,
    [fixture.notification.envKeys.partnerStateFile]: fixture.storageStates.forwarder,
    [fixture.notification.envKeys.requestId]: fixture.requests.freight.id,
...
async function main() {
  ...
  const env = mergedEnv(fileEnv, {
    E2E_ALLOW_REMOTE_MARKETPLACE_NOTIFICATION: "true",
  ...
  if (!isLocalOrAllowedRemoteUrl(baseUrl, "MARKETPLACE_NOTIFICATION")) failures.push(`E2E_BASE_URL is not allowed. origin=${safeOrigin(baseUrl)}`);
  if (!isLocalOrAllowedRemoteUrl(supabaseUrl, "MARKETPLACE_NOTIFICATION")) failures.push(`SUPABASE_URL is not allowed. origin=${safeOrigin(supabaseUrl)}`);
  ...
  console.log("Marketplace notification staging E2E runner");
  ...
  runStep("ready", "node", ["scripts/check_marketplace_notification_e2e_readiness.mjs"], env);
  runStep("notification-e2e", "node", ["scripts/e2e_marketplace_notification_dashboard.mjs"], env);
...
try {
  ...
  console.error("Marketplace notification staging E2E runner failed");
