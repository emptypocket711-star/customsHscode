...
begin
  ...
  if jsonb_array_length(coalesce(v_line.required_corrections, '[]'::jsonb)) > 0 then
    raise exception '남은 보정 항목이 있어 HS 요청을 생성할 수 없습니다.';
  end if;

  if nullif(trim(coalesce(v_line.product_name, '')), '') is null then
    raise exception '품명이 없어 HS 요청을 생성할 수 없습니다.';
  end if;
