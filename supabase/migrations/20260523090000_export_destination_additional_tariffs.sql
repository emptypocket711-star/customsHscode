create table if not exists public.export_destination_additional_tariffs (
  id uuid primary key default gen_random_uuid(),
  country_code text not null,
  destination_hs_code text not null,
  additional_tariff_code text not null,
  tariff_program text not null,
  rate_text text,
...
create unique index if not exists export_destination_additional_tariffs_unique_idx
  on public.export_destination_additional_tariffs(country_code, destination_hs_code, additional_tariff_code, coalesce(origin_country_code, ''), source_version);

create index if not exists export_destination_additional_tariffs_lookup_idx
  on public.export_destination_additional_tariffs(country_code, destination_hs_code, origin_country_code, effective_from, effective_to, status);

create index if not exists export_destination_additional_tariffs_source_idx
  on public.export_destination_additional_tariffs(source_version, status);

alter table public.export_destination_additional_tariffs enable row level security;

create policy "published export destination additional tariffs readable by app users" on public.export_destination_additional_tariffs
  for select to anon, authenticated using (status = 'published');

create policy "staff writes export destination additional tariffs" on public.export_destination_additional_tariffs
